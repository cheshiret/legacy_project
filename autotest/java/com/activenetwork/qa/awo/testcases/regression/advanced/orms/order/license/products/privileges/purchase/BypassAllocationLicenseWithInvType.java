...
/**
 * @Description:This test case was designed to verify error message for outfitter customer when bypass purchase allocation license with inventory type
 * @Preconditions:
 * BypassAllocationInd should set as True in X_PROP table;
 * @LinkSetUp:
 * d_assign_feature:id=6122,6132
 * d_hf_add_cust_profile:id=3150,3160,3170,3180
 * d_hf_add_hunt_location:id=120,130
 * d_hf_add_allo_type_licyear:id=40
 * d_hf_priv_invtype_licyear:id=140
 * d_hf_add_allo_pri_inv:id=120
 * d_hf_add_privilege_prd:id=3050
 * d_hf_add_pricing:id=4492
 * d_hf_add_prvi_license_year:id=3180
 * d_hf_assi_pri_to_store:id=2270
 * d_hf_add_qty_control:id=2250
 * d_hf_add_outfitter_alloc:id=40
 * d_hf_assoc_cust_store:id=40
 * @SPEC: Purchase Privilege - Allocation Licence - Bypass Allocation License: True [TC:117983]
 * @Task#: Auto-2151
 * 
 * @author Jane Wang
 * @Date  May 15, 2014
 */
public class BypassAllocationLicenseWithInvType extends LicenseManagerTestCase {
	...
	public void wrapParameters(Object[] param) {
		...
		login.location = "AB - Call Center Manager - Auto/BEAUMONT HUSKY(Store Loc)";
		
		schema = TestProperty.getProperty(env + ".db.schema.prefix") + "AB";
		
		privilege.code = "BPP";
		privilege.name = "ByPassPrivilege";
		...
		String loc01 = "854";
		String loc02 = "855";
		option01 = loc01+" (Available: " + getAvailableQtyForOutfitterAllocation(outfitterCust01.custNum, alloType, privilege.licenseYear, loc01, schema)+")";
		option02 = loc02+" (Available: " + getAvailableQtyForOutfitterAllocation(outfitterCust01.custNum, alloType, privilege.licenseYear, loc02, schema)+")";
	}
	...
	private void setOutfitterInfoAndLocation(String outfitterNum, String location, boolean selectLocation) {
		...
		if (pages == selectFulfillmentLicWidget) {
			logger.info("Fulfillment method has been configured, select 'Select Immediately' as default..");
		...
		logger.info("Select privilege inventory as default.");
