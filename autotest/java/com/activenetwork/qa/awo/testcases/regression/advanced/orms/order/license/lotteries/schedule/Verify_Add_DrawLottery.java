...
/**
 * @Description:This case is used to verify add a schedule with 'draw lottery' config
 ...
 * @Date  March 07, 2014
 */
public class Verify_Add_DrawLottery extends LicenseManagerTestCase{
	...
	private void verifyScheduleInfoInList() {
		...
		passed &= MiscFunctions.compareResult("Algorithm",	"Draw Lottery", scheduleInfo.get(3));
	...
	private void verifyEditableStatusForItem() {
		...
		passed &= MiscFunctions.compareResult("Seed Number:", true, detailsPage.isSeedNumberEditable());
		passed &= MiscFunctions.compareResult("Initial Number:", true, detailsPage.isInitalNumberEditable());
	...
	public void wrapParameters(Object[] param) {
		...
		schedule.setLicenseYear(String.valueOf(DateFunctions.getYearAfterCurrentYear(8)));
		schedule.setDescription("Verify add Draw Lottery");
		schedule.setLottery("Hunt product");
		schedule.setSeedNumber("3");
		schedule.setInitialNumber("5");
		schedule.setCalculateAgeMethod("Specific Date");
		schedule.setSpecificDate(DateFunctions.getDateAfterToday(8*365+122));
		schedule.setFreezePeriodEndDate(DateFunctions.getDateAfterToday(8*365+123));
		schedule.setFreezePeriodEndTime("11:11");
		schedule.setFreezePeriodEndAmPm("AM");
		schedule.setAwardAcceptanceByDate(DateFunctions.getDateAfterToday(8*365+128));
