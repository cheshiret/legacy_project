...
public class VerifySalesLocationField extends ResourceManagerTestCase{
	...
	public void execute() {
		...
		rptCriteriaPg.selectSalesChannel("Field");
		rptCriteriaPg.waitLoading();
		rm.VerifyReportSelectionCriteria("SalesChannel", salesChannel, "Field", salesChannel.get(0));
...
//		rm.VerifyReportSelectionCriteria("SalesLocation", salesLocation, "All", salesLocation.get(0));
	...
	public void reportSectionInfo(){
		...
		salesLocation.add("ADELAIDE CAMPGROUND (UT.)");
		salesLocation.add("AMANITA (ID)");
		salesLocation.add("BASAM CABIN (CO)");
		//List<String> salesLocation = new ArrayList<String>();
