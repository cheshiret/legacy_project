...
public class VerifySelectionCriteriaForPOS extends ResourceManagerTestCase{
	...
	private void verifySelectionCriteriaCorrect(){
		...
		if(!rptCriteriaPg.getAllBalanceType().replaceAll("#", "").equals("Balance Owing")){
		...
		if(rptCriteriaPg.checkPaymenttatusExists()){
			throw new ErrorOnPageException("Payment Status should not be Visible.");
		}
		if(rptCriteriaPg.checkOrderStatusExists()){
			throw new ErrorOnPageException("Order Status should not be Visible.");
		}
