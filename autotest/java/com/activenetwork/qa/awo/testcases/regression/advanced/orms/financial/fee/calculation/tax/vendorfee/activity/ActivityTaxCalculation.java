package com.activenetwork.qa.awo.testcases.regression.advanced.orms.financial.fee.calculation.tax.vendorfee.activity;
...
/**
 * 
 * @Description: verify the Tax fee schedule applied on activity Vendor Fee calculated correctly
 ...
 * @SPEC: Calculate Taxes - Activity Orders [TC:116038] Step 1, 3
 * @Task#: AUTO-2122
 * 
 * @author qchen
 * @Date  May 8, 2014
...
public class ActivityTaxCalculation extends LicenseManagerTestCase {
	...
	private FeeScheduleData vendorFeeSchdl = new FeeScheduleData();
	...
	public void execute() {
		...
		vendorFeeSchdl.feeSchdId = lm.addNewFeeSchedule(vendorFeeSchdl);
		LicMgrFeeScheduleSearchPage.getInstance().changeScheduleStatus(vendorFeeSchdl.feeSchdId, OrmsConstants.ACTIVE_STATUS);
	...
	public void wrapParameters(Object[] param) {
		...
		taxSchdl.feeType = "Vendor Fee";//IMPORTANT
		...
		vendorFeeSchdl.productCategory = "Activity";
		vendorFeeSchdl.feeType = taxSchdl.feeType;//IMPORTANT
		vendorFeeSchdl.productGroup = taxSchdl.productGroup;
		vendorFeeSchdl.product = taxSchdl.product;//IMPORTANT
		vendorFeeSchdl.effectStartDate = taxSchdl.startDate;
		vendorFeeSchdl.effectEndDate = taxSchdl.endDate;
		vendorFeeSchdl.applicableTaxes.add(taxSchdl.taxName);//IMPORTANT
		vendorFeeSchdl.applyRate = "Per Unit";
		vendorFeeSchdl.rateAmount = String.valueOf(NumberUtil.getRandomDouble(10, 100));
		
		//IMPORTANT: expected tax amount
		taxAmount = new BigDecimal(Double.parseDouble(vendorFeeSchdl.rateAmount) * Double.parseDouble(taxSchdl.rate) / 100).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
	}
}
