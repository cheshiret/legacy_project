 ...
 * @Date  Jun 18, 2012
 */
public class SearchWithMultiCondition extends InventoryManagerTestCase {
    ...
    public void execute(){
    	...
    	// 1.search by status and supplier name
		boolean result = true;
		...
		result &= this.verifySearchResult("Status", purchaseOrderInfo.searchStatus);
		result &= this.verifySearchResult("Supplier Name", purchaseOrderInfo.searchSupplierName);
		String creationDate = posPurchaseOrderPg.getColumnByName("Order Creation Date").get(0);

    	// 2.search by status and creation date
		purchaseOrderInfo.searchSupplierName = "";
		...
		purchaseOrderInfo.searchCreationEndDate = DateFunctions.getDateAfterGivenDay(creationDate, 0);
		posPurchaseOrderPg.searchPOPurchaseOrder(purchaseOrderInfo);
		result &= this.verifySearchResult("Status", purchaseOrderInfo.searchStatus);
		
		// 3.search by supplier name and creation date
		purchaseOrderInfo.searchStatus = "";
		purchaseOrderInfo.searchSupplierName = "456";
		purchaseOrderInfo.searchCreationStartDate = DateFunctions.getDateAfterGivenDay(creationDate, 0);
		purchaseOrderInfo.searchCreationEndDate = DateFunctions.getDateAfterGivenDay(creationDate, 0);
		posPurchaseOrderPg.searchPOPurchaseOrder(purchaseOrderInfo);
		result &= this.verifySearchResult("Supplier Name", purchaseOrderInfo.searchSupplierName);
		result &= this.verifySearchResult("Order Creation Date", purchaseOrderInfo.searchCreationStartDate);
		result &= this.verifySearchResult("Order Creation Date", purchaseOrderInfo.searchCreationEndDate);
    ...
    private boolean verifySearchResult(String searchBy, String searchValue){
    	...
    	if("Status".equalsIgnoreCase(searchBy)){
        	...
        	// search by PO#
        	if("PO #".equalsIgnoreCase(searchBy) || "Supplier Name".equalsIgnoreCase(searchBy)){
        	...
        	} else if("Order Creation Date".equalsIgnoreCase(searchBy)){
    ...
    private List<String> getPONumFromDBByStatus(){
    	...
    	if(!StringUtil.isEmpty(purchaseOrderInfo.searchCreationStartDate)){
    		query = query.concat(" and to_char(create_date, 'MM/dd/yyyy') >= '"+DateFunctions.formatDate(purchaseOrderInfo.searchCreationStartDate, "MM/dd/yyyy")+"'");
    	}
    	if(!StringUtil.isEmpty(purchaseOrderInfo.searchCreationEndDate)){
    		query = query.concat(" and to_char(create_date, 'MM/dd/yyyy') <= '"+DateFunctions.formatDate(purchaseOrderInfo.searchCreationEndDate, "MM/dd/yyyy")+"'");
    	}
