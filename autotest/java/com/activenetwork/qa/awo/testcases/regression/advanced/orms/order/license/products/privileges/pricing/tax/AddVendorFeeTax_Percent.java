...
/**
 * 
 * @Description: Verify add Tax info on Vendor fee--percent
 * 
 * @Preconditions:1. privilege product:107
 * 
 * @LinkSetUp: 
 *                     
 * @SPEC: Add Product Pricing - Tax info - Vendor Fee Tax [TC:117360] 
 *        Add Product Pricing - Tax info - Privilege - DB check [TC:117361] 
 * 
 * @Task#: AUTO-2183
 * 
 * @author szhou
 * @Date  May 6, 2014
 */
public class AddVendorFeeTax_Percent extends LicenseManagerTestCase {
	...
	public void execute() {
		...
		lm.gotoAddTaxPage(pricing.id,false);
		//verify priceInfo
		this.verifyTaxInfoForVendorFee(pricing.vendorTaxes, false);
		//verify in DB
		this.verifyTaxInfoInDB(pricing.vendorTaxes, pricing.prodCode, FEETYPE_VENDORFEE, false, schema);
	...
	private void verifyTaxInfoInDB(List<TaxInfo> vendorTaxes,String prdCD,String feeType, boolean isFlat,String schema){
		...
		for (int i = 0; i < vendorTaxes.size(); i++) {
			...
			if(isFlat){
				...
				result &= MiscFunctions.compareResult("Tax amount",String.valueOf(new BigDecimal(expectResult.getAccount(0)[1])) , String.valueOf(new BigDecimal(actualResult.getRate())));
			}else{
