...
/**
 * @Description:This case is used to verify ui for add a lottery schedule with 'Instant Lottery' configuration
 * @LinkSetUp:  d_hf_add_lottery_execution_config:id=120(DESCRIPTION='InstantLotteryConf')
 * 			    d_hf_add_lottery_prd:id=540 -- For instant lottery schedule
 * @SPEC:[TC:044737] Add Priviledge Lottery Schedule 
 * @Task#: Auto-2064
 * @author Phoebe Chen
 * @Date  March 03, 2014
 */
public class Verify_Add_InstantLottery  extends LicenseManagerTestCase{
	...
	private void verifyEditableStatusForItem() {
		...
		passed &= MiscFunctions.compareResult("Freeze Period End Date:", false, detailsPage.isFreezePeriodEndDateExist());
		passed &= MiscFunctions.compareResult("Freeze Period End Time:", false, detailsPage.isFreezePeriodEndTimeExist());
		passed &= MiscFunctions.compareResult("Award Acceptance By Date:", false, detailsPage.isAwardAcceptanceByDateExist());
		passed &= MiscFunctions.compareResult("Award Acceptance By Time:", false, detailsPage.isAwardAcceptanceTimeExist());
		if(!passed){
	...
	private void verifyScheduleInfoInList() {
		...
		passed &= MiscFunctions.compareResult("Algorithm",	"Instant Lottery", scheduleInfo.get(3));
	...
	public void wrapParameters(Object[] param) {
		...
		schedule.setExecutionConfig("InstantLotteryConf");//Check point, do not change
		schedule.setLicenseYear(String.valueOf(DateFunctions.getYearAfterCurrentYear(6)));
		schedule.setDescription("Verify add instance Draw Lottery");
		schedule.setLottery("For instant lottery schedule");
		schedule.setCalculateAgeMethod("Specific Date");
		schedule.setSpecificDate(DateFunctions.getDateAfterToday(6*365+123));
		schedule.setFreezePeriodEndDate(DateFunctions.getDateAfterToday(6*365+125));
		schedule.setAwardAcceptanceByDate(DateFunctions.getDateAfterToday(6*365+127));
