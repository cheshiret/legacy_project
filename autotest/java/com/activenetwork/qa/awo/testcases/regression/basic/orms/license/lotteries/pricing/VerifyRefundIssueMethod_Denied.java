 ...
 * 			
 * 			  d_hf_add_lottery_prd:id=440(CODE='P10') 
 * 			  d_hf_add_pricing:id=4132 --lottery
 * 			  d_hf_add_lottery_quantity_control:id=340
 *            d_hf_add_lottery_schedule:id=250 
 *            d_hf_assign_lottery_to_store:id=400
 *            d_hf_assi_hunts_to_lottery:id=410,570
 * 			  d_hf_add_hunt_license_year:id=990,1090
 * @SPEC:[TC:056841] Change Refund Issue Method - during Lottery Execution 
 * @Task#: Auto-2056
 * @author Phoebe Chen
 * @Date  March 14, 2014
 */
public class VerifyRefundIssueMethod_Denied extends LicenseManagerTestCase{
	...
	public void execute() {
		...
		this.declineLotteryToOrderCartPg(ordNum);
	...
	private void verifyRefundInfoInHistoryPg() {
		...
		ReservationHistory historyInfo = historyPage.getOrderHistoryByTransaction("Reallocate Payment");
		if(!historyInfo.info.matches(".*PER CHQ refund.*")){
			throw new ErrorOnPageException("The refund issue method is not correct!");
		}
		logger.info("The refund issue method is correct!");
	}
	
	private void declineLotteryToOrderCartPg(String ordNum) {
		...
		lm.declinedAwardLotteryFromAddItemPgToCustDetailPg(cust.residencyStatus, null, ordNum);
		if(addItemPg.exists()){
			lm.goToCart();
		}
	...
	public void wrapParameters(Object[] param) {
		...
		//lottery info
		lottery.setCode("P10");
		lottery.setDescription("Lottery prodcut for pricing 10");
		...
		schedule.setDescription("PrdSchedule10");
