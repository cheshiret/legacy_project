...
/**
 * @Description: Duplicate registration when reverser a vehicle registration to order cart page.
 * Verify error message.
 * @Preconditions:
 * @SPEC: Duplicate vehicle registration [TC:004988]
 * @Task#: Auto-1790
 * @LinkSetUp:  d_hf_add_cust_profile:id=2420
 * d_hf_add_pricing:id=3592
 * 
 * @author nding1
 * @Date  Dec 4, 2013
 */
public class DuplicateRegWhenReverse extends LicenseManagerTestCase{
	...
	private LicMgrVehicleDetailPage detailPg = LicMgrVehicleDetailPage.getInstance();
	private OrmsSystemErrorWidget errorWidget = OrmsSystemErrorWidget.getInstance();
	...
	public void execute() {
		...
		// reverse vehicle transaction to order cart page
		lm.gotoVehicleOrderDetailPage(ordNum);
		lm.reverseVehicleOrderItemOnOrdDetailsPg(vehicle.operationReason, vehicle.operationNote, vehicle.registration.product, vehicle.registration.purchaseType);
		
		// duplicate this registration
		this.duplicateReg();
		this.verifyErrorMsg();
		lm.logOutLicenseManager();
	}
	
	private void duplicateReg(){
		orderCartPg.clickSearch("Vehicles");
		lm.gotoVehicleDetailsPgByMiNum(vehicle.registration.miNum);
		detailPg.clickRegistration();
		ajax.waitLoading();
		browser.waitExists(errorWidget, orderCartPg, detailPg);
	}
	...
	public void wrapParameters(Object[] param) {
		...
		vehicle.hullIdSerialNum = "DupReverse" + DateFunctions.getCurrentTime();
