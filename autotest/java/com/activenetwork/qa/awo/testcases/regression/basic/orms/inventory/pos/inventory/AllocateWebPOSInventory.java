 ...
 * 				3. Go to IM, allocate the POS to web.
 ...
 * 
 * @SPEC: TC043062:Allocate Web POS Inventory
 ...
 * @Date  Sep 17, 2012
 */
public class AllocateWebPOSInventory extends InventoryManagerTestCase {
	...
	@Override
	public void execute() {
		// if POS is not assigned in Fnm, assign the product to Web.
		...
		// Go to Inventory Manager to allocate and deallocate to/from Web.
	...
	private void deallocatePOS(){
		logger.info("Deallocate all POS from Web.");
		im.deallocatePosProductToWeb(pos);
		im.deallocatePosProductToWeb(pos1);
		im.deallocatePosProductToWeb(pos2);
	}

	private void verifyAllocate(POSInfo pos){
		logger.info("Verify allocate status of POS:"+pos.product);
		
		//Check point 1: check allocate POS to Web
		im.allocatePosProductToWeb(pos);
		this.checkAllocateStatus(pos);
		
		//Check point 2: re-allocate that POS to Web
		if("Product Package".equalsIgnoreCase(pos.productRelactionshipType)) {
			...
			expectMessage = "The POS Product < "+pos.product+" > has already been allocated for the Web. Please unselect it.";
		}
		im.allocatePosProductToWeb(pos);
		this.verifyErrorMsg();
	}
	...
	private void verifyErrorMsg(){
		...
		String errorMsg = webAllocationPage.getMessage();
