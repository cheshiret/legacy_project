package com.activenetwork.qa.awo.testcases.regression.advanced.orms.financial.fee.calculation.tax.activityfee;
...
/**
 * 
 * @Description: verify the Tax fee schedule applied on Activity Fee calculated correctly
 ...
 * @SPEC: Calculate Taxes - Activity Orders [TC:116038] Step 1, 2
 * @Task#: AUTO-2122
 * 
 * @author qchen
 * @Date  May 8, 2014
...
public class ActivityTaxCalculation extends LicenseManagerTestCase {
	...
	private FeeScheduleData activityFeeSchdl = new FeeScheduleData();
	...
	public void execute() {
		...
		//2. add a new Activity Fee schedule
		lm.gotoFeeScheduleListPageFromTopMenu();
		activityFeeSchdl.feeSchdId = lm.addNewFeeSchedule(activityFeeSchdl);
		LicMgrFeeScheduleSearchPage.getInstance().changeScheduleStatus(activityFeeSchdl.feeSchdId, OrmsConstants.ACTIVE_STATUS);
		...
		//4. goto fee details page to verify the Tax applied on Activity Fee calculated correctly
	...
	public void wrapParameters(Object[] param) {
		...
		taxSchdl.feeType = "Activity Fee";//IMPORTANT
		...
		activityFeeSchdl.productCategory = "Activity";
		activityFeeSchdl.feeType = taxSchdl.feeType;//IMPORTANT
		activityFeeSchdl.productGroup = taxSchdl.productGroup;
		activityFeeSchdl.product = taxSchdl.product;//IMPORTANT
		activityFeeSchdl.effectStartDate = taxSchdl.startDate;
		activityFeeSchdl.effectEndDate = taxSchdl.endDate;
		activityFeeSchdl.applicableTaxes.add(taxSchdl.taxName);//IMPORTANT
		activityFeeSchdl.applyRate = "Per Unit";
		activityFeeSchdl.rateAmount = String.valueOf(NumberUtil.getRandomDouble(10, 100));
		
		//IMPORTANT: expected tax amount
		taxAmount = new BigDecimal(Double.parseDouble(activityFeeSchdl.rateAmount) * Double.parseDouble(taxSchdl.rate) / 100).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
	}
}
