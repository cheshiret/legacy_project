 ...
 * 				  2. Privilege: rpm-renewPrivilege, pricing of which has value 'Yes' on attribute 'Applies to All State/Province'.
 ...
 * @Task#:@Task#:AUTO-1242 Renew Privilege PCR 2940: TC047822(303 feature)
 * 
 * @author pzhu
 * @Date  Oct 31, 2012
 */

public class RenewPrivilegeOfAllStatePricing extends LicenseManagerTestCase{
	...
	private String totalPriceForAll;
	...
	public void execute() {
		...
		if(StringUtil.compareNumStrings(this.totalPriceForAll, amount_NY)!=0)
		{
			throw new ErrorOnPageException("Total price of renew privilege of customer from 'New York' is wrong... ", this.totalPriceForAll, amount_NY);
		}else{
			logger.info("Check total price of renew privilege of customer from 'New York' Passed!!!");
		}
		
		if(StringUtil.compareNumStrings(this.totalPriceForAll, amount_UT)!=0)
		{
			throw new ErrorOnPageException("Total price of renew privilege of customer from 'Utah' is wrong... ", this.totalPriceForAll, amount_UT);
		}else{
	...
	public void wrapParameters(Object[] param) {
		...
		privilege.purchasingName = "rpm-renewPrivilege";
		...
		this.totalPriceForAll = lm.getPriCustPrice(schema, privilege.code, RENEWAL_PURCHASE_TYPE_ID, OrmsConstants.STATUS_ACTIVE);//"20";//Total price comes from table D_HF_ADD_PRICING, ID=3842
	}
}
