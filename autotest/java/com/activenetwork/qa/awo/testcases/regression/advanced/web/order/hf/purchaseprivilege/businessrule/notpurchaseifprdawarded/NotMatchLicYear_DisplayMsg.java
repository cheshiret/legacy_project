 ...
 * 3. the product "SR4 - HFSK PrivRule004" exists:
 * a. has 2 license year records: current and next years
 * b. has a business rule "Cannot purchase IF parameter product awarded" - Not Match License Year / Display warning 4015; Parameter product: SR6 - HFSK UnlockPrivRule006 
 * @LinkSetUp:
 * d_hf_add_privilege_prd:id=2350,2370
 * d_hf_add_pricing:id=3242,3262
 * d_hf_assi_pri_to_store:id=1600,1620
 * d_hf_add_prvi_license_year:id=2420,2430,2460,2470
 * d_hf_add_qty_control:id=1580,1600
 * d_hf_add_pri_business_rule:id=380
 * d_web_hf_signupaccount:id=1100
 ...
 * @SPEC: Rule setup: Not Match Licence Year / Display Warning 4015 [TC:102415]
 * @Task#: Auto-1866
 * 
 * @author Lesley Wang
 * @Date  Sep 12, 2013
 * @Note from Sara, UWP-1513, for Licence Cross Reference, the waitingPeriod = "Lifetime"
 */
public class NotMatchLicYear_DisplayMsg extends
	...
	public void execute() {
		...
		// Go to HFSK to make sure the privileges "SR4" and "SR6" with different license years can be added to shopping cart.
		hf.invokeURL(url);
		...
		// In Web, verify the 1 awarded unlocked privilege shown and the privilege "SR4" with any license year shown a message
		hf.invokeURL(url);
		hf.lookupAccountToAcctOverviewPg(cus);
		...
		this.verifyWarningMsgOnPrdDetailsAndListPg(true, true);
		hf.signOut();
		...
		// In Web, verify no awarded unlocked privilege shown, but SR4 privilege with any license years are shown
		hf.invokeURL(url);
		hf.lookupAccountToAcctOverviewPg(cus);
		hf.gotoLicenseCategoriesListPg(cus.residencyStatus, cus.identifier);
		this.updateExpectedDisplayResult(true, true, true, true, false, false);
		catListPg.filterPrivAndVerifyPrivInCategoryListPg(privileges, displayed);
		this.verifyWarningMsgOnPrdDetailsAndListPg(false, false);
		hf.signOut();
		
		// import a record with next license year
		this.prepareUnlockedPriv(fileName, unlockedPriv2, cus.custNum, hunt);
		
		// In Web, verify only 1 awarded unlocked privilege shown, and the privilege "SR4" with any license year shown a message
		hf.invokeURL(url);
		hf.lookupAccountToAcctOverviewPg(cus);
		hf.gotoLicenseCategoriesListPg(cus.residencyStatus, cus.identifier);
		...
		// Verify the privilege "SR4" with any license year shown and purchased successfully
		hf.gotoLicenseCategoriesListPg(cus.residencyStatus, cus.identifier);
		this.updateExpectedDisplayResult(true, true, true, true, false, false);
		catListPg.filterPrivAndVerifyPrivInCategoryListPg(privileges, displayed);
		this.verifyWarningMsgOnPrdDetailsAndListPg(false, false);
	...
	public void wrapParameters(Object[] param) {
		// Customer info
		cus.fName = "PurchaseRule04_FN";
		cus.lName = "PurchaseRule04_LN";
		...
		cus.identifier.identifierNum = "1R9Y4x4165";
		...
		// Privilege info
		privilege.code = "SR4";
		privilege.name = "HFSK PrivRule004";
		...
		fileName = "BR_NotMatchLicYear_Display";
