...
/**
 * 
 * @Description:(P) Verify (For Group type)
 ...
 * @Date  Jun 6, 2014
 */
public class SubmissionFlowForGroup extends HFABLotteryApplicationTestCase{
	...
	public void execute() {
		...
		huntsPg.setupHuntChoices(hunt1,hunt2,hunt3,hunt4);
		hfab.gotoLotteryDetailsPgFromAddHuntsPg();
		hfab.applyLotteryAsGroupLeaderToAddHuntsPg();
		huntsPg.verifyHuntChoices(null,null,null,null);

		//Setup hunt choices with Continue button, the entered will be retained
		huntsPg.setupHuntChoicesWithContinueBtn(hunt1,hunt2,hunt3,hunt4);
		hfab.gotoLotteryDetailsPgFromAddHuntsPg();
		hfab.applyLotteryAsGroupLeaderToAddHuntsPg();
		huntsPg.verifyHuntChoices(hunt1,hunt2,hunt3,hunt4);

		//Verify selected hunts in shopping cart page
		hfab.chooseLotteryHuntsToCart(Arrays.asList(cusNew), hunt1,hunt2,null,null);
		shoppingCartPg.verifyLotteryHuntChoices(lottery.getLegalName(), hunt1.getHuntCode(), hunt2.getHuntCode());
	...
	public void wrapParameters(Object[] param) {
		cus.custNum = hfab.getCustomerNumByEmail("hfab_00013@webhftest.com", schema);
		cus.email = cus.custNum;
		cus.residencyStatus = RESID_STATUS_ALBERTA;

		cusNew.custNum = hfab.getCustomerNumByEmail("hfab_00014@webhftest.com", schema);
		cusNew.fName = "ABLotteryApp_FN33";
		cusNew.lName = "ABLotteryApp_LN33";
		...
		huntInfo = initializeEnteredLotteryHuntAttris(true, hunt1,hunt2);
	}
