...
/**
 * @Description: This case was designed for verify edit a privilege pricing   
 * @Preconditions:
...
public class Edit extends LicMgrEditProductPricingTestCase {
	...
	public void execute() {
		...
		productPricingPage = lm.gotoProductPricingPageFromListPage(pricing.prodType,
				pricing.prodCode);
		...
		PricingInfo oldPricing = lm.getProductPricingInfo(pricing.id,
				productPricingPage);
		result &= verifyPricingAddUpdateInfo(oldPricing);
		
		// c. verify system adds a new Product Pricing with information 
		PricingInfo newPricingUI = lm.getProductPricingInfo(newPricing.id,
				productPricingPage);
	...
	public void resetPricingInfo() {
		newPricing.prodCode = "B02";
		newPricing.prodType = "Privilege";
		...
		newPricing.purchaseType = "Original";
