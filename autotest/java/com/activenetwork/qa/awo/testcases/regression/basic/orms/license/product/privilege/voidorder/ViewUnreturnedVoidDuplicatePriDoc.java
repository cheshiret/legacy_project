 ...
 * @Date  2012-3-28
 */
public class ViewUnreturnedVoidDuplicatePriDoc extends LicenseManagerTestCase {
	...
	public void execute() {
		...
		// process order(don't print)
		ordNum = lm.processOrderCart(pay,false);
		
		// duplicate, print
		lm.replacePrivilegeToCartFromCustQuickSearch(cust, ordNum, privilege);
	...
	private void verifyTransTypes(String[] transTyps2, String ordNum) {
		...
		if (transTyps2.length != typList.size()) {
			throw new ErrorOnPageException("There should only "
					+ transTyps2.length + " privilige instances");
		}
	...
	private void searchReturnDocumentByOrNum(String orderNum) {
		LicMgrPrivilegeReturnDocumentPage returnDocPage = LicMgrPrivilegeReturnDocumentPage
				.getInstance();
		logger.info("Search privilege return document from LicMgrPrivilegeReturnDocumentPage");
	...
	public void wrapParameters(Object[] param) {
		...
		transTyps = new String[] { "Duplicate", "Duplicate"};
	}
