...
/**
 * @Description: When edit a lottery schedule,
 ...
 * @Preconditions: 1.Need a lottery exist.(d_inv_new_lottery_program, ID:480, name: VerifyMsgAcceptancePeriod)
 * 2.'Requires Customer Acceptance' must be set to Yes in lottery detail page.
 * 3.Lottery Schedule is "Active" and Lottery Schedule currently not on-going.(handle in case)
 * @SPEC: Edit Lottery Schedule - Acceptance Period validation [TC:044750]
 * @Task#: Auto-1343
 * 
 * @author nding1
 * @Date  Jan 10, 2013
...
public class VerifyErrMsg_AcceptancePeriodValidation extends InventoryManagerTestCase {
	...
	public void execute() {
		...
		// 2.Customer Acceptance Period To date and time is blank.
		lotterySchedule.customerAcceptFromDate = DateFunctions.getDateAfterToday(34, tz);
		...
		// 4.Customer Acceptance Period From date later than To date.
		lotterySchedule.customerAcceptToDate = DateFunctions.getDateAfterToday(33, tz);// using boundary value.
		this.getMessageAndVerify(exceptMsg_FromEqualLaterTo);
		
		// 5.Customer Acceptance Period From date and time is before the Execution date and time plus Freeze Duration.
		lotterySchedule.customerAcceptFromDate = DateFunctions.getDateAfterToday(31, tz);// Don't change!
		lotterySchedule.customerAcceptToDate = DateFunctions.getDateAfterToday(38, tz);
		this.getMessageAndVerify(exceptMsg_FromEqualBeforeExe);
		
		// 6.Customer Acceptance Period From date and time is equal to the Execution date and time plus Freeze Duration.
		lotterySchedule.customerAcceptFromDate = DateFunctions.getDateAfterToday(32, tz);// Don't change!
		this.getMessageAndVerify(exceptMsg_FromEqualBeforeExe);
	...
	public void wrapParameters(Object[] param) {
		...
		lotterySchedule.appStartDate = DateFunctions.getDateAfterToday(20, tz);
		lotterySchedule.appEndDate = DateFunctions.getDateAfterToday(27, tz);
		lotterySchedule.web = true;
		lotterySchedule.executeDate = DateFunctions.getDateAfterToday(29, tz);// Don't change!
