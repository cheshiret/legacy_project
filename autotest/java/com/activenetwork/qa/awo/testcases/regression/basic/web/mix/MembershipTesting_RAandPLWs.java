 ...
 * @author vzhao
 */
public class MembershipTesting_RAandPLWs extends WebTestCase
{
	/**
	 * Script Name   : <b>MembershipTesting_RAandPLWs</b>
	 * Generated     : <b>Nov 26, 2009 11:58:28 PM</b>
	 * Description   : Functional Test Script
	...
	public void execute() {
		// create one RA customer account 
		web.invokeURL(url);
		...
		while (!dpIter.dpDone()) {
			...
			try {
				logger.info("Sign in with RA account for state: " + stateName);
				if(!plwUrl.startsWith("els")){ //Sara[6/18/2014] Per Aida, we don't have ELS anymore as part of our PL (private labels)	
					...
					web.signIn(cust.email, cust.password); // verify whether the new account can sign in success
					web.signOut();
		...
		// only create one PLW customer account in SC and verify whether or not can log into RA
		this.scTestData();
	...
	public void wrapParameters(Object[] param) {
		...
		url =TestProperty.getProperty(env + ".web.ra.url");// ra's url
		String email = web.generateNewWebEmail("ra", env); //for RA site
	...
	private void scTestData(){
		scUrl = TestProperty.getProperty(env + ".web.sc.url");// create a sc PLW account
		String email = web.generateNewWebEmail("rapl", env); 
		String pw = web.readQADB("OLD_PWD");
