 ...
 *              check point:1. reactive vehicle title from transferable status work flow
 ...
 * @Date Jun 18, 2012
 */
public class ReactiveVehicleTitle_FromTransferable extends LicenseManagerTestCase {
	...
	public void execute() {
		...
		// set to transferable
		lm.gotoVehicleDetailsPgByMiNum(vehicle.registration.miNum);
		lm.setTitleToTransferableByPrd(vehicle.title.product);	
		
		// reactive title
		lm.reactiveTitleByPrd(vehicle.title.product);
		// verify vehicle title status in the vehicle detail page
        this.verifyStatusForVehicle(ACTIVE_STATUS, vehicle.title.product);
		...
		this.verifyTransactionInfoInDB(transinfo, schema, orderNum, TRANTYPE_REACTIVATE_VEHICLE_TITLE);
	...
	private void verifyStatusForVehicle(String status,String product){
		...
		if(!status.equals(vehicle.title.status)){
			throw new ErrorOnPageException("Status is not correct...",status,vehicle.title.status);
		}
	...
	private void verifyVehicleInstInfoInDB(Vehicle v,
		...
		result &= MiscFunctions.compareResult("Title ID", v.title.titleId, info[0]);
	...
	private void addVehicleProduct(VehicleRTI vrti,String locationClass,PricingInfo... prices){
		logger.info("Add vehicle product : "+vrti.getPrdName()+" for data prepared...");
	...
	public void wrapParameters(Object[] param) {
		...
		regVehicleRTI.setPrdCode("RVR");
		regVehicleRTI.setPrdName("ReactiveVehicleTitleR");
		...
		titleVehicleRTI.setPrdCode("RTT");
		titleVehicleRTI.setPrdName("ReactiveVehicleTitleT");
		...
		cust.identifier.identifierNum = "4896875129";
		cust.customerClass = "Individual";
		cust.dateOfBirth = "Aug 09 1985";
		cust.lName = "TEST-ReactiveVclTitle1";
		cust.fName = "QA-ReactiveVclTitle1";
