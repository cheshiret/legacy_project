 ...
 * @LinkSetUp: D_HF_ADD_PRIVILEGE_PRD:id=3350
 *             D_HF_ASSI_PRI_TO_STORE:id=2550
 *             D_HF_ADD_PRVI_LICENSE_YEAR:id=3460
 *             D_HF_ADD_QTY_CONTROL:id=2530
...
public class CalculateTax_Transfer extends LicenseManagerTestCase{
	...
	public void execute() {
		...
		vendorTaxSchdID_tax1 = this.getTaxSchdID(privilege.privilegeId, FEETYPE_VENDORFEE, TRANSFER_PURCHASE_TYPE_ID,pricing_transfer.vendorTaxes.get(0).getTaxName(),schema);
		//get vendor fee tax 2 schedule id
		vendorTaxSchdID_tax2 = this.getTaxSchdID(privilege.privilegeId, FEETYPE_VENDORFEE, TRANSFER_PURCHASE_TYPE_ID,pricing_transfer.vendorTaxes.get(1).getTaxName(),schema);
		...
		priFeesPage.verifyFeeInfo(pricing_transfer.vendorTaxes.get(0).getTaxName(), vendorTaxSchdID_tax1, vendorTaxAmount_1, TRANNAME_TRANSFER_PRIVILEGE);
		priFeesPage.verifyFeeInfo(pricing_transfer.vendorTaxes.get(1).getTaxName(), vendorTaxSchdID_tax2, vendorTaxAmount_2, TRANNAME_TRANSFER_PRIVILEGE);
		lm.gotoOrderCartPageFromFeeDetailPage();
		//verify tax total amount by tax name
		ordCartPg.verifyFeeAmountCorrect(pricing_transfer.vendorTaxes.get(0).getTaxName(), true, String.valueOf(vendorTaxAmount_1));
		ordCartPg.verifyFeeAmountCorrect(pricing_transfer.vendorTaxes.get(1).getTaxName(), true, String.valueOf(vendorTaxAmount_2));
	...
	public void wrapParameters(Object[] param) {
		...
		privilege.name = "TaxProcessPrivilege9";
		privilege.code = "TP9";
		...
		tax_vendor_1.setPurchaseType(TRANSFER_PURCHASE_TYPE_ID);
		
		TaxInfo tax_vendor_2 = pricing_transfer.new TaxInfo();
		...
		tax_vendor_2.setPurchaseType(TRANSFER_PURCHASE_TYPE_ID);
		vendorTaxAmount_2 = ((new BigDecimal(pricing_transfer.vendorFee).
				multiply(new BigDecimal(tax_vendor_2.getRate()).movePointLeft(2))).
				setScale(2,BigDecimal.ROUND_HALF_UP)).doubleValue();
		
		pricing_transfer.vendorTaxes.add(tax_vendor_1);
		pricing_transfer.vendorTaxes.add(tax_vendor_2);
