 ...
 * (2) Privilege product 104 should have 'Original' pricing, license year and business rule;
 * 
 * @author Jane
 *
 */
public class LM_MSVeriFone_NewCustAndPrivSale extends MSVeriFoneTestCase {
	
	private String receiptNum, ordNum, tan;
	private String originalSaleType = "Original";
	private String saleLoc = "WAL MART 305(332)";
	private String custIdenType = "Social Security Number";
	private Properties originalResponse = new Properties();
	private Properties voidResponse = new Properties();
	
	public void execute() {
				
		//construct new customer and privilege original sale nodesinfo
		logger.info("Step#1 create a new customer and purchase privilege via Verifone.");
		initialNewCustAndPrivSaleRequestNodesInfo();
...
//		System.out.println("Response for original sale:\n"+response.toString());
		...
		comparePrivilegeOriginalPurchaseOrderDetailsInfo(originalResponse);

		//construct void privilege sale nodesinfo
		logger.info("Step#2 void privilege via Verifone.");
...
//		System.out.println("Response for void sale:\n"+response.toString());
		...
		lm.gotoCustomerOrderPage();
		comparePrivilegeOriginalPurchaseOrderDetailsInfo(voidResponse);
	...
	public void wrapParameters(Object[] param) {
		...
		cust.fName = "QA-VeriFone" + cust.seq;
		cust.lName = "Test-VeriFone" + cust.seq;
		cust.dateOfBirth = "1978-01-01";
		...
		cust.identifier.identifierNum = NumberUtil.getUniqueSSN();
		
		privilege.code = "110";//as original 104 product was removed from verifone device,change to another product
		privilege.licenseYear = lm.getFiscalYear(schema);
	}
	
	private void initialNewCustAndPrivSaleRequestNodesInfo() {
		...
		nodesInfo.put(c_tan, DateFunctions.getDateStamp(true)+DateFunctions.getLongTimeStamp()+"00008");//check if it was mandatory or not
		
		nodesInfo.put(c_fName, cust.fName);	
		nodesInfo.put(c_lName, cust.lName);
		nodesInfo.put(c_dob, DateFunctions.formatDate(cust.dateOfBirth, "yyyyMMdd"));
		nodesInfo.put(c_gender, convertGender(cust.custGender));//M
		nodesInfo.put(c_ethnicity, convertRace(cust.ethnicity));//1
		nodesInfo.put(c_phone, cust.hPhone);
		nodesInfo.put(c_email, cust.email);
		nodesInfo.put(c_addr, cust.address);
		nodesInfo.put(c_zip, cust.zip);
		nodesInfo.put(c_country, convertAbbreviations(cust.country));//USA
		nodesInfo.put(c_city, cust.city);
		nodesInfo.put(c_state, convertAbbreviations(cust.state));
		nodesInfo.put(c_ssn, cust.identifier.identifierNum);//ssn
		nodesInfo.put(c_residency, "R");//residency indicator
		nodesInfo.put(c_add_res_proof, "1");//TODO valid values
	...
	private Properties parseResponseForOriginalSaleOrderInfo(List<Map<String, String>> response) {
		...
		responsePro.put("purchase_type", originalSaleType);
...
//		responsePro.put("transaction_number", receiptNum);
	...
	private void comparePrivilegeOriginalPurchaseOrderDetailsInfo(Properties responsePro) {
