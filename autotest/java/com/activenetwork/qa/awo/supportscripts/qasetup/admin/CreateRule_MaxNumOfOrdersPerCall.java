...
import com.activenetwork.qa.awo.supportscripts.function.admin.ormsclientrulefunctions.MaxNumOfOrdersPerCallRuleFunction;
import com.activenetwork.qa.awo.util.AwoUtil;
import com.activenetwork.qa.testapi.util.StringUtil;
import com.activenetwork.qa.testapi.util.TestProperty;

public class CreateRule_MaxNumOfOrdersPerCall extends SetupCase {
	...
	public void executeSetup() {
		new MaxNumOfOrdersPerCallRuleFunction().execute(new Object[] {login, ruleData});
	}
	...
	public void wrapParameters(Object[] param) {
		...
		dataTableName = "d_rule_max_n_ords_per_call";
		ids = "";
	}
	...
	public void readDataFromDatabase() {
		...
		//rule data info
		ruleData = new RuleDataInfo("Maximum Number Of Orders Per Call");//IMPORTANT
		...
		if (null != datasFromDB.get("StartDate") && datasFromDB.get("StartDate").length() > 0) {
			ruleData.startDate = datasFromDB.get("StartDate");
		}
		if (null != datasFromDB.get("EndDate") && datasFromDB.get("EndDate").length() > 0) {
			ruleData.endDate = datasFromDB.get("EndDate");
		}
		if (null != datasFromDB.get("EffectiveDate") && datasFromDB.get("EffectiveDate").length() > 0) {
			ruleData.effectiveDate = datasFromDB.get("EffectiveDate");
		}

		//Maximum Number of Orders Per Call specific attributes
		ruleData.maximumNumberOfOrdersPerCall = datasFromDB.get("MaxNumOfOrdersPerCall");
	}
}
