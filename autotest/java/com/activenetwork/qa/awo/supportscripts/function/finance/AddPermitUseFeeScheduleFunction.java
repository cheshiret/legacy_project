 ...
 * @Date  Nov 21, 2012
 */
public class AddPermitUseFeeScheduleFunction extends FunctionCase {
	...
	public void execute() {
		...
		if(!fd.feeSchdId.matches("\\d+")){// error message existed.
			throw new ErrorOnPageException(fd.feeSchdId);
		}
		newAddValue = fd.feeSchdId;
		feeMainPg.changeScheduleStatus(fd.feeSchdId, OrmsConstants.ACTIVE_STATUS);
		if(!feeMainPg.isFeeScheduleActive(fd.feeSchdId)){
			throw new ErrorOnPageException("Failed to add new permit use fee schedule.");
		}
	}
}
