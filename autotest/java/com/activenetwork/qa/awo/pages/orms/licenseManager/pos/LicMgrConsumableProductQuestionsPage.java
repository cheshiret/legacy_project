 ...
 * @Date  May 27, 2011
 */
public class LicMgrConsumableProductQuestionsPage extends LicMgrConsumableProductDetailsPage implements ILicMgrProductQuestionPage{
	
	private static LicMgrConsumableProductQuestionsPage _instance = null;
	
	private LicMgrConsumableProductQuestionsPage() {}
	
	public static LicMgrConsumableProductQuestionsPage getInstance() {
		if(null == _instance) {
			_instance = new LicMgrConsumableProductQuestionsPage();
		}
	...
	public boolean exists() {
		return browser.checkHtmlObjectExists(".class", "Html.A", ".text", "Add Question") && browser.checkHtmlObjectExists(".class", "Html.SELECT", ".id", new RegularExpression("ConsumableProductView-\\d+\\.productSubCategory", false));
	}
	...
	public void checkShowCurrentRecordsOnly() {
		browser.selectCheckBox(".id", new RegularExpression("^SearchPrdQuestionAssignmentCriteria-\\d+\\.showCurrent", false));
		ajax.waitLoading();
	}
	
	public void uncheckShowCurrentRecordsOnly() {
		browser.unSelectCheckBox(".id", new RegularExpression("^SearchPrdQuestionAssignmentCriteria-\\d+\\.showCurrent", false));
		ajax.waitLoading();
	}
	...
	public void selectStatus(String status) {                    
		browser.selectDropdownList(".id", new RegularExpression("^SearchPrdQuestionAssignmentCriteria-\\d+\\.status", false), status);
	}
	
	public void unSelectStatus(){
		browser.selectDropdownList(".id", new RegularExpression("^SearchPrdQuestionAssignmentCriteria-\\d+\\.status", false), 0);
	}
	/**
	...
	public void setDisplayText(String displayText) {
		browser.setTextField(".id", new RegularExpression("^SearchPrdQuestionAssignmentCriteria-\\d+\\.displayText", false), displayText);
	}
	...
	public void selectLocationClass(String locationClass) {
		browser.selectDropdownList(".id", new RegularExpression("^SearchPrdQuestionAssignmentCriteria-\\d+\\.locationClassID", false), locationClass);
	}
	
	public void unselectLocationClass(){
		browser.selectDropdownList(".id", new RegularExpression("^SearchPrdQuestionAssignmentCriteria-\\d+\\.locationClassID", false), 0);
	}
	...
	public void setSearchCriteria(QuestionInfo question){
		this.setSearchCriteria(question.status, question.questDisplayText, question.locationClass);
	}
	
	public void setSearchCriteria(String status,String displayText,String locClass){
	...
	public void searchQuestios(String status,String displayText,String locClass){
	...
	public boolean questionAssignmentIDExists(String questionAssignmentID){
		boolean result;
		IHtmlObject[] objs = browser.getHtmlObject(".class", "Html.A", ".text", questionAssignmentID);
		if(objs.length ==1 ){
			result =  true;
		}else{
			result =  false;
	...
	/**
	 * Get product question assignment id identified by assignment attributes - display text/location class/license year from/
	 * license year to/effective from date/effective to date
	 * @param question
	 * @return
	 */
	public String getProductQuestionAssignmentID(QuestionInfo question) {
		IHtmlObject objs[] = browser.getTableTestObject(".id", "product_question_assignment");
		if(objs.length < 1) {
			throw new ItemNotFoundException("Can't find Product Question Assignment table object.");
		}
		...
		for(int i = 0; i < table.rowCount(); i ++) {
			if(table.getCellValue(i, 1).equalsIgnoreCase("Active")
					&& table.getCellValue(i, 2).equalsIgnoreCase(question.questDisplayText) 
					&& table.getCellValue(i, 3).equalsIgnoreCase(question.locationClass) 
					&& table.getCellValue(i, 4).equalsIgnoreCase(question.licenseYearFrom) 
					&& table.getCellValue(i, 6).equalsIgnoreCase(DateFunctions.formatDate(question.effectiveFromDate, "E MMM d yyyy"))) {
...
//		}
	 ...
	 */
	public String getProductQuestionId(int columnIndex, String value) {
		IHtmlObject objs[] = browser.getTableTestObject(".id", "product_question_assignment");
		if(objs.length < 1) {
			throw new ItemNotFoundException("Can't find Product Question Assignment table object.");
		}
		...
		IHtmlTable table = (IHtmlTable)objs[0];
		for(int i = 0; i < table.rowCount(); i ++) {
			if(table.getCellValue(i, columnIndex).equalsIgnoreCase(value)) {
				return table.getCellValue(i, 0);
			}
		}
		
		logger.warn("Couldn't find cell by given value:<" + value + "> in column <" + columnIndex + ">");
	 ...
	 * @param question
	 * @return Map<LocationClass, ID> - if Location Class=All, the length of map is 1; if not, the length will be greater than 1.
	 */
	public Map<String, String> getProductQuestionAssignmentLocationClassIDMap(QuestionInfo question) {
		Map<String, String> locationClassesAndIDs = new HashMap<String, String>();//key is location class, value is id
		if(null!= question.locationClass && question.locationClass.length()>0) {
			locationClassesAndIDs.put(question.locationClass, this.getProductQuestionAssignmentID(question));
		}
	...
	/**
	 * Click consumable question assignment id to go to question assignment detail widget
	 * @param id
	 */
	...
	/**
	 * This method used to verify given question ids sorted correct by given order
	 * @param ids
	 * @param isAsc
	 * @return
	 */
	public boolean verifyQuestionSortedCorrect(List<String> ids,boolean isAsc){
		return this.verifyTableRecordsDisplaySort(".id", "product_question_assignment", 0, ids, isAsc);
	}
