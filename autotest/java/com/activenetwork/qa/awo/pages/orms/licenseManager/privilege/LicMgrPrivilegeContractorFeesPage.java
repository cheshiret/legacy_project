 ...
 * @Date Dec 17, 2011
 */
public class LicMgrPrivilegeContractorFeesPage extends
		LicMgrPrivilegeProductDetailsPage {
	private static LicMgrPrivilegeContractorFeesPage instance = null;

	private LicMgrPrivilegeContractorFeesPage() {
	}

	public static LicMgrPrivilegeContractorFeesPage getInstance() {
		if (instance == null) {
			instance = new LicMgrPrivilegeContractorFeesPage();
		}
		return instance;
	}
	...
	public RaFeeScheduleData getFeeSchInfo(String scheduleID) {
		...
		schedule.effectDate = table.getCellValue(row, 5);
		schedule.tranType = table.getCellValue(row, 6);
		schedule.baseRate = table.getCellValue(row, 7);
		schedule.otherRate = table.getCellValue(row, 8);
	...
	public void verifyScheduleListInfo(RaFeeScheduleData schedule) {
		...
		if (schedule.locationClass != null
			...
			if (!schedule.locationClass.replaceFirst("\\d+", "").trim()  //Vivian [20140709]
		...
		if (schedule.otherRate != null && !schedule.otherRate.equals("")) {
			if (!schedule.otherRate.equalsIgnoreCase(actually.otherRate)) {
				throw new ItemNotFoundException("Group Rate "
						+ actually.otherRate + " not same with given value "
						+ schedule.otherRate);
			}
		}
