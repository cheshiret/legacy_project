...
/**
 * 
 * @Description: This page is one of sub pages in Privilege Product page
 * @Preconditions:
 * @SPEC: <<Assign Product to Stores thru Location Class.doc>> & <<Unassign Product from Stores thru Location Class.doc>>
 * @Task#: Auto-510
 * 
 * @author qchen
 * @Date  May 16, 2011
 */
public class LicMgrPrivilegeProductStoreAssignmentsPage extends LicMgrPrivilegeProductCommonPage implements ILicMgrProductAgentAssignmentsPage {
	
	private static LicMgrPrivilegeProductStoreAssignmentsPage _instance = null;
	
	protected  LicMgrPrivilegeProductStoreAssignmentsPage() {}
	
	public static LicMgrPrivilegeProductStoreAssignmentsPage getInstance() {
		if(null == _instance) {
			_instance = new LicMgrPrivilegeProductStoreAssignmentsPage();
		}
	...
	public void clickAssignToStoresInSelectedLocationClasses() {
		browser.clickGuiObject(".class", "Html.A", ".text", new RegularExpression("Assign to (Agents|Issuers|Locations) in Selected Location Classes", false)); //"Assign to Agents in Selected Location Classes");
		ajax.waitLoading();
	}
...
//		HtmlObject objs[] = browser.getTableTestObject(".className", "gridView");
//		if(objs.length == 0) {
//			throw new ErrorOnPageException("Can't find Store Assignments table object.");
//		}
...
//		Browser.unregister(objs);
//		return targetNum;
		return this.getAgentAssignmentValueByLocationName(locationClass, 4);
	}
