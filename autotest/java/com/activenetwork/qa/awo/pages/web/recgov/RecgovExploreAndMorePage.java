 ...
 * @Date  Dec 26, 2011
 */
public class RecgovExploreAndMorePage extends RecgovCommonPage{
	private static RecgovExploreAndMorePage _instance=null;

	public static RecgovExploreAndMorePage getInstance() {
		if(null==_instance) {
			_instance=new RecgovExploreAndMorePage();
		}

		return _instance;
	}

	public RecgovExploreAndMorePage() {}

	/**
	 * Check whether this page exist according to the DIV with title (More Great Resources to Explore:)
	 */
	public boolean exists() {
		return browser.checkHtmlObjectExists(".class", "Html.DIV", ".text", new RegularExpression("^Explore More Great Adventures.*",false));
	}
	...
	public String[] getExploreWays(){
		Property[] p1=Property.toPropertyArray(".class","Html.UL",".text",new RegularExpression("^AutoTouring.*",false));
		RegularExpression reg = new RegularExpression(".*marketing\\.do\\?goto=acm\\/Explore_And_More.*", false);
