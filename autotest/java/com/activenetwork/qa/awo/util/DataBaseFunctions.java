...
public class DataBaseFunctions {
	 ...
	 * 
	 * @param schema
	 *            - if null, return US/Eastern by default
	 * @param parkName
	 *            - if null, return contract level time zone
	 * @return
	 */
	public static String getTimeZoneString(String schema, String parkName) {
		...
		if (parkName == null || parkName.length() == 0) {
			...
			sql = "select * from D_LOC where UPPER(NAME) = UPPER('" + parkName + "') and LEVEL_NUM=40";//40----Facility
		}
		// if schema are not specified,
		// will choose the US/Eastern time
		if (schema != null && schema.trim().length() > 0) {
	...
	public static void setValueFromQAAutomationTable(String var,String val, String env) {
		db.resetSchema("qa_user");
		String setClause=(StringUtil.isEmpty(env)?"val":"val_"+env)+"='"+val+"'";
		String query = "update qa_automation set "+setClause+"where var=\'" + var
				+ "\'";
	...
	public static void recordPageTime(String pageName, int toolID, int time) {
		...
		try {
			String runningId = TestProperty.getProperty("runningId");
			String env = TestProperty.getProperty("target_env");
			...
			String queryInsert = "insert into page_timing (pagename,loadingtime,buildversion,testcasename,env,project,recorddate,execution_id,tool) values "
					+ "('" + pageName + "'," + time + ",'"
					+ build + "','" + fullCaseName + "','" + env + "','" + project
					+ "'," + "sysdate,'" + runningId + "'" + "," + toolID
					+ ")";
	 ...
	 * 
	 * @param caseName
	 *            - the test case full name. For example:
	...
	public static int getTestCaseID(String caseName) {
		...
		String query = "select id from test_cases where UPPER(casename)=UPPER('"
				+ caseName + "')";
...
//		String query = "select seq('email_seq') as seq";//mysql
	 ...
	 */
//	public static void recordBuildNumber(String runningid, String buildNumber) {
//		db.resetDefaultDB();
//
...
//	}
	...
	public static int getSiteTypeID(String siteType, String schema) {
		...
		String query="select prd_grp_id as id from p_prd_grp where UPPER(prd_grp_name) = '"+siteType.toUpperCase()+"'";
	...
	/**
	 * Record the EWeb session IDs to AWO_SESSIONS and TEST_EXECUTION_DETAILS tables
	 * @param caseID
	 * @param executionID
	 * @param session_ids
	 * @param new_session
	 */
	public static void recordSessionID(int caseID, String executionID, String session_ids, String new_session) {
		db.resetDefaultDB();
		
		String query="insert into awo_sessions (case_id,case_name,machine,session_id, create_date) values ("+caseID+ ",'"+TestProperty.getProperty("fullCaseName")+"','"+SysInfo.getHostName()+"("+SysInfo.getHostIP()+")','"+new_session+"',sysdate)";
		try {
		...
		if (caseID > 0 && !StringUtil.isEmpty(executionID) && executionID.matches("\\d{14}")) {			
			query = "update test_execution_details set "
					+ "session_id='"
					+ session_ids
	...
	public static int getLoadDispatch() {
		db.resetDefaultDB();
		db.connect();
//		String query = "select load_dispatch.nextval as value from dual";//oracle
...
//		db.resetSchema(getSchemaName("common", TestProperty.getProperty("target_env")));
//		String sql = "select LABEL_VALUE from All_X_TRANSLATION where LABEL_KEY = '"
//				+ key + "' and contract='"+contract+"'";
		db.resetSchema(getSchemaName(contract, TestProperty.getProperty("target_env"))); //Lesley[20140114]: update due to no such table in Live_Common
	...
	public static String getLicenseFiscalYearTranslatedLabel(String contract) {
		return getTranslatableLabelValue("translatable.licensefiscalyear", contract);
	}
	...
	public static String getStateVendorFeeTranslatedLabel(String contract) {
		return getTranslatableLabelValue("translatable.statevendorfee", contract);
	}
	
	public static String getTransFeeTranslatedLabel(String contract) {
		return getTranslatableLabelValue("translatable.transactionfee", contract);
	}
	...
	public static String getLatestAvailableTourDate(String schema, String parkName, String tourName) {
		String today=DateFunctions.getToday("yyyy-MM-dd");
		return getLatestAvailableTourDate(schema, parkName, tourName,today,1);
	}
	
	public static String getLatestAvailableTourDate(String schema, String parkName, String tourName, String startDate, int num) {
		db.resetSchema(schema);
		startDate=DateFunctions.formatDate(startDate, "yyyy-MM-dd");
		String query="select to_char( min(iti.date_time), 'YYYY-MM-DD') as adate from i_tour_inv iti, d_loc dl, p_prd pp where UPPER(dl.name)='"+parkName.toUpperCase()+"' and dl.id=iti.loc_id and iti.prd_id=pp.prd_id and UPPER(pp.prd_name)='"+tourName.toUpperCase()+"' and status=1 and date_time>=to_date('"+startDate+"','YYYY-MM-DD') and iti.tot_avail>="+num;
		String aDate=db.executeQuery(query, "adate", 0);
		
		if(StringUtil.isEmpty(aDate)) {
			throw new ItemNotFoundException("No tour inventory found for tour '"+tourName+"' after "+startDate);
		} else {
			logger.info("Tour '"+tourName+"' has at least "+num+" available inventory on "+aDate);
		}
...
//	public static Properties getTranslations(String schema, String contract_abbr) {
//		db.resetSchema(schema);
//		String query="select label_key, label_value from all_x_translation where contract=UPPER('"+contract_abbr+"') or contract='CNTR'";
//		List<String[]> list=db.executeQuery(query, new String[]{"label_key","label_value"});
//		
...
//		db.resetSchema(schema);
//		String query="select contract,label_key, label_value from all_x_translation";
//		List<String[]> list=db.executeQuery(query, new String[]{"contract","label_key","label_value"});
//		
//		Properties trans=new Properties();
//		for(String[] ss:list) {
//			trans.setProperty(ss[0], ss[1]);
...
//	}
	...
	public static String getTranslations(String schema, String label_key) {
		db.resetSchema(schema);
		String query="select label_value from x_translation where label_key='"+label_key+"'";
	 ...
	 * @param result
	 */
	public static void updateResultForDataTable(String tableName, String id, String env, int result) {
		logger.info("Update table "+tableName+" for id "+id+" "+env+"_result to "+result);
		
		db.resetDefaultDB();
		String query="update "+tableName+" set "+env+"_result="+result+" where id="+id;
		db.executeUpdate(query);
	}
	
	public static void updateValueForDataTable(String tableName, String id, String env, String value) {
		logger.info("Update table "+tableName+" for id "+id+" "+env+"_value to "+value);
		
		db.resetDefaultDB();
		String query="update "+tableName+" set "+env+"_value='"+value+"' where id="+id;
		db.executeUpdate(query);
	}
	
	public static String getTCsIDinSpiraTeam(int script_id) {
		db.resetDefaultDB();
		String query="select tc_number from spira_auto where script_id="+script_id;
		try {
			return db.executeQuery(query, "tc_number", 0);
		} catch(Exception e) {
			AutomationLogger.getInstance().warn("Failed to get spira tc number due to "+e);
			return null;
		}
	}
	
	public static void insertDataIntoDB(String tableName, String tableColNames, List<String[]> values){
		db.resetDefaultDB();
		for(int i=0; i<values.size(); i++){
			String[] value = values.get(i);
			String colValues = StringUtil.EMPTY;
			for(int j=0; j<value.length; j++){
				colValues += "'"+value[j]+"'"+ (j==value.length-1?"":",");
			}
			String sql = "insert into "+tableName+" ("+tableColNames+") values ("+colValues+")";
