...
public class VerifyDisplay_ReportDateLocationIsFacility extends ResourceManagerTestCase{
	...
	public void execute() {
		...
		rptCriteriaPg.selectFacilityID(rd.park);
        ...
        //End Date minus Start Date is equal 31 days         
		//---needs to run for maximum reporting period of 31 days if the report is run for a facility/park,
        //rd.startDate = DateFunctions.getDateAfterToday(-31);
        //rd.endDate = DateFunctions.getDateAfterToday(0);
        //rm.verifyReportDate(rd.startDate, rd.endDate, errorMessage.get(5));
		// End Date minus Start Date is greater than 31 days
	...
	public void reportSectionInfo(){
		...
		errorMessage.add("Reporting period exceeds the maximum report period of 31 days for the selected location. Please refine the search criteria.");
	}
