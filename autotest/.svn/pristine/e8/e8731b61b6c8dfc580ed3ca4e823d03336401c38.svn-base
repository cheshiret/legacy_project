...
/**
 * @Description: It is used to verify the new Vehicle order is updated in the cart correctly when title a vehicle
 * firstly, then renew a vehicle registration.
 * @Preconditions:
 ...
 * 3. Make sure the vehicle title product "TIT-TitleBoat" exists.
 ...
 * @Date  Jun 13, 2012
 */
public class Renew_UpdateOrder extends
...
//	private RegistrationInfo regisInfo = new RegistrationInfo();
	...
	public void execute() {
		...
		// Title the vehicle and add the order into order cart
		lm.gotoVehiclesDetailsPgBySerialNum(vehicle.hullIdSerialNum);
		lm.titleVehicleToOrderCartFromVehicleDetailPage(vehicle);
		
		// Go to search vehicle page from order cart page
		lm.gotoVehiclesSearchPgFromOrderCartPg();
		...
		boolean result = true;
		//result &= lmOrderCartPg.verifyTransactionName("Title Vehicle,Renew Registration");
		result &= lmOrderCartPg.verifyTransactionName("Title,Renew Registration");
