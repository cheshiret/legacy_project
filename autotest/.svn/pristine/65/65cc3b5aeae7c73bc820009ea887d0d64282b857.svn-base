...
/**
 * @Description:Rule Name:Customer must be AT LEAST parameter age on the date of privilege purchase
 ...
 *                3.Privilege must have business rule: Customer must be AT LEAST parameter age on the date of privilege purchase
 *                (d_hf_add_pri_business_rule, ID: 260)
 ...
 * @Date  Mar 7, 2012
 */
public class AtLeastParameterAgeOnPurchasePrivilegeDate extends LicenseManagerTestCase {
	...
	public void wrapParameters(Object[] param){
		...
		cust.fName = "QA-TransferRule3";
		cust.lName = "TEST-TransferRule3";// IMPORTANT this customer must be older than 20.
		cust.residencyStatus = "Non Resident";
		cust.identifier.identifierNum = lm.getCustomerNumByCustName(cust.lName, cust.fName, schema);

		privilege.code = "995";
		privilege.purchasingName = "995-TransferAtLeastParamAge";
		...
		toCust.fName = "QA-TransferRule33";
		toCust.lName = "TEST-TransferRule33";// IMPORTANT this customer must be younger than 20 and older than 10.
		...
		ruleName = "Customer must be AT LEAST parameter age on the date of Privilege purchase";
	...
	private void updateDOBForCust(Customer customer, int days){
		...
		customer.dateOfBirth = DateFunctions.calculateDate(DateFunctions.getToday(DataBaseFunctions.getContractTimeZone(schema)),  paramAge*(-1), days);
