 ...
 *                 2. The paraments privilege product is inactive VPH-HisPrivilegeOrder.
 ...
 * @Date  Feb 26, 2012
 */
public class BuyXWithInactiveY extends LicenseManagerTestCase{
	private String switchLocation, paraterPrivCode,paraterPriv, paraterPrivCount;
	...
	private LicMgrPrivilegeProductDetailsPage productDetailPg = LicMgrPrivilegeProductDetailsPage.getInstance();
	public void execute() {
		lm.checkPrivilegesExist(schema, paraterPrivCode);
		lm.checkPrivilegesExist(schema, privilegeInfo.code);
		lm.loginLicenseManager(login);
		lm.gotoPrivilegeListPageFromTopMenu();
		
		lm.gotoPrivilegeDetailsPageFromProductListPage(paraterPrivCode,false);
		if(productDetailPg.checkPrivlegeActive()){
			lm.deactivatePrivilegeProduct(paraterPrivCode);
		}
	...
	public void wrapParameters(Object[] param) {
		...
		privilegeInfo.code = "IXW";
		privilegeInfo.name = "ParaterPerTransaction";
		...
		paraterPrivCode = "IXL";
		paraterPriv ="IXL-ParaterPerTransaction";
		paraterPrivCount = "0";
