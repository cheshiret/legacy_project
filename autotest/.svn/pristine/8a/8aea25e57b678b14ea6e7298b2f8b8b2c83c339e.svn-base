 ...
 * b. "LH21 - Leftover sale hunt 21" - Limited / has quota added (Leftover sale quota 08); has two license year records: current and next years
 ...
 * 3. the product "SR1 - HFSK PrivRule001" exists:
 ...
 * 4. "Leftover sale quota 08" quota type has two license year records:current and next years 
 * @LinkSetUp:
 * d_hf_add_privilege_prd:id=2320,2370
 * d_hf_add_pricing:id=3212,3262
 * d_hf_assi_pri_to_store :id=1570,1620
 * d_hf_add_prvi_license_year :id=2360,2370,2460,2470
 * d_hf_add_qty_control:id=1550,1600
 * d_hf_add_pri_business_rule:id=350
 ...
 * @SPEC: Rule setup: Match Licence Year / Don't display [TC:102403]
 * @Task#: Auto-1866
 * 
 * @author Lesley Wang
 * @Date  Sep 12, 2013
 */
public class MatchLicYear_NotDisplay extends
	...
	public void execute() {
		...
		// Go to HFSK to make sure the privileges "SR1" and "SR6" with different license years can be added to shopping cart.
		hf.invokeURL(url);
		...
		// In Web, verify the 1 awarded unlocked privilege shown and the privilege "SR1" with current license year doesn't shown
		hf.invokeURL(url);
		hf.lookupAccountToAcctOverviewPg(cus);
		...
		// In Web, verify only 1 awarded unlocked privilege shown, and the privilege "SR1" with current license year shown, but with next year not shown
		hf.invokeURL(url);
		hf.lookupAccountToAcctOverviewPg(cus);
		hf.gotoLicenseCategoriesListPg(cus.residencyStatus, cus.identifier);
		...
		// Purchase the privilege "SR1" with current license year successfully
		...
		// Verify the privilege "SR1" with next license year shown and purchased successfully
		hf.gotoLicenseCategoriesListPg(cus.residencyStatus, cus.identifier);
	...
	public void wrapParameters(Object[] param) {
		...
		// Privilege info
		privilege.code = "SR1";
		privilege.name = "HFSK PrivRule001";
		...
		fileName = "BR_MatchLicYear_NotDisplay";
	}
