 ...
 * @Date  Dec 6, 2011
 */
public class VerifyRAFeeSchdSelectCreationForPrivilege extends FinanceManagerTestCase{
	...
	public void execute() {
	    ...
	    this.verifyRAFeeScheduleCreationForPrivilege();
	    
	    //logout finance manager
	    fnm.logoutFinanceManager();
	}
	
	private void verifyRAFeeScheduleCreationForPrivilege(){
		FinMgrRaFeeSchMainPage schMainPg = FinMgrRaFeeSchMainPage.getInstance();
		
		logger.info("Verify RA fee schedule creation for Privilege.");
	...
	private void verifyProduct(){
		...
		List<String[]> productGrpDB=fnm.getProductGroupNameUsingFeeSchdDetailPgFromDB(schema, "10", "");
		List<String[]> productDB=fnm.getProductNameUsingFeeSchdDetailPgFromDB(schema, "10", "", "", "1");
