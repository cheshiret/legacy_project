 ...
 * 3. update hunt application type to Group, that means un-select Individual
 * 4. verify Individual hunt permit is inactive by automatic
 *    verify Group member hunt permit is active
 *    verify Group Leader hunt permit is active
 * 5. active Individual hunt permit, will pop error message
 * @Preconditions:
 * @SPEC:   TC:046000 Step1 TC:045957
 * @Task#:Auto-1254
...
public class PermitInfoAndMsgByAppChangeInd extends LicenseManagerTestCase{
	...
	public void execute() {		
		...
		//unSelect hunt application type individual
		this.unSelectHuntApplicantTypeWithIndividual();		
		
		//group member 
		lm.gotoHuntPermitListPgFromHuntDetailPg();
		lm.filterHuntPermit(OrmsConstants.ACTIVE_STATUS);
		//in active list, individual hunt permit info should be not existing
		this.verifyHuntPermitIDWhetherExistingAtList(huntPermits.get(0).getHuntPermitID(), false);
		// in active list, group leader hunt permit info should be existing
		this.verifyHuntPermitIDWhetherExistingAtList(huntPermits.get(1).getHuntPermitID(), true);
		// in active list, group member hunt permit info should be existing
		...
		//in inactive list, individual hunt permit info should be existing
		this.verifyHuntPermitIDWhetherExistingAtList(huntPermits.get(0).getHuntPermitID(), true);
		//in inactive list, group leader hunt permit info should be not existing
		this.verifyHuntPermitIDWhetherExistingAtList(huntPermits.get(1).getHuntPermitID(), false);
		// in inactive list, group member hunt permit info should be not existing
		this.verifyHuntPermitIDWhetherExistingAtList(huntPermits.get(2).getHuntPermitID(), false);
		
		//when activate individual hunt permit info will pop message
	...
	public void wrapParameters(Object[] param) {
		...
		hunt.setHuntCode("PerInfoByAppI");
		hunt.setDescription("PerInfoByAppI");
...
//		huntPermitGroupLeader.setPermit("PH1","PrivilegeForHuntPermit_1");
		huntPermitGroupLeader.setPermit("PrivilegeForHuntPermit_1");
...
//		huntPermitGroupMember.setPermit("PH1","PrivilegeForHuntPermit_1");
		huntPermitGroupMember.setPermit("PrivilegeForHuntPermit_1");
		...
		+ " does not allow Hunt Permits with Applicant Type of \"Individual\".";
	}
	...
	private void unSelectHuntApplicantTypeWithIndividual(){
		huntDetailPg.unSelectApplicantType("Individual");
