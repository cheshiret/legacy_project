 ...
 *                4.PrivilegeReplacementTransactionCoverage is set to 5(handle in case)
 *                  (update X_PROP set value=5 where name='PrivilegeReplacementTransactionCoverage';)
 *                  5 means Replacement of an individual Privilege Instance in an Order replaces ALL Privilege Instances in the Order.
 *                  The price of the Replacement shall take into account the price for replacing ALL of the Privilege Instances in the Order that were replaced.
 * @SPEC: Step 7~9 of TC:042692
 * @Task#:Auto-1243
 * 
 * @author nding1
 * @Date  Sep 14, 2012
 */
public class GetListOrderTotalIndividualLM extends LicenseManagerTestCase {
	private PrivilegeInfo pri1 = new PrivilegeInfo();
	private PrivilegeInfo pri2 = new PrivilegeInfo();
	
	public void execute() {
		// setup precondition No.4
		lm.updatePriReplacementTransactionCoverage(schema, "5");
...
//		String orderNum = lm.processOrderCart(pay).split(" ")[0];
	...
	private void verifyProductAndPricing(String orderNum){
		...
		totalPrice = totalPrice.add(new BigDecimal(lm.getPriCustPrice(schema, pri1, REPLACEMENT_PURCHASE_TYPE_ID, OrmsConstants.STATUS_INACTIVE)));
		...
		// verify
		replacementPg.verifyDupPriPricingPerOrder(orderNum, purchasingNameList, totalPrice.setScale(2).toString());
	}
}
