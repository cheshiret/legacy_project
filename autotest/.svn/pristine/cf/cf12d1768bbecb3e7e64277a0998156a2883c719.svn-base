...
/**
 * @Description: 1,Register vehicle A(Boat). 2, Make title of vehicle B(Motor). 3, Go to vehicle detail page. 4. select existing vehicle B(motor) and assign to A. 
 * 				 5, Go to vehicle order detail page, unassign motor B from A.	
 * 				    
 * @Preconditions: 
...
public class AddExistToParent extends LicenseManagerTestCase {
	...
	public void execute() {
		...
		//Step 2: make title of child vehicle(motor) order.
		...
		childVehicle.title.titleNum = omOrdSumPg.getTitleNum();
		lm.finishOrder();
		
		
		//Step 3: go to parent vehicle detail page, and add child vehicle to parent.
		this.addExistingChildVehicleToParent();
	...
	private void addExistingChildVehicleToParent() {
		LicMgrVehicleMotorsPage motorsPg = LicMgrVehicleMotorsPage.getInstance();
		
		//Demo text of existing motors in the drop down list: CHILD8447374T6168100, YAMA, 2012, 200.0 (137640326)
		motorString = this.childVehicle.getSerialNum()+", "+this.childVehicle.getManufacturerName()+", "+this.childVehicle.getModelYear()+", "+this.childVehicle.getHorsePower()+" ("+this.childVehicle.id+")";
		logger.info("Existing Motors to be child vehicle is -->"+motorString);
		
		lm.gotoVehicleDetailsPgByMiNum(this.parentVehicle.registration.miNum);
		motorsPg.waitLoading();
		
		
		childVehicle.setCustomerExistingMotor(motorString);
		childVehicle.status = "Active";
		lm.addVehicleMotor(childVehicle);
	...
	public void wrapParameters(Object[] param) {
		...
		cust[0] = new Customer();
		cust[0].lName = "TEST-RAFee2";
		cust[0].fName = "QA-RAFee2";
