...
public class VerifyQuestionDetail extends LicenseManagerTestCase {
	...
	public void execute() {
		...
		// The Question Display Text is not specified, i.e. left null/blank.
		// (message 1 in the Spec)
		errormess &= this.verifyAddQuestionField("Prompt", null);
		// The Product is of Product Category "Privilege", and no Purchase Type
		// has been selected.(message 8,9 in the Spec)
//		errormess &= this.verifyAddQuestionField("Purchase Type", "Don't Ask");
		errormess &= this.verifyAddQuestionField("Purchase Type", "Optional");//DEFECT-48349 fixed this issue
		// The Collection Method is not specified, i.e. left null/blank.(message
		// 12 in the Spec)
		...
		// License/Fiscal Year To entry is specified but entry for
		// License/Fiscal Year From is greater than the entry for License/Fiscal
		// Year To.(message 3 in the Spec)
		...
		lm.addQuestionsForProduct(question, false);
		// Effective To Date entry is specified but entry for Effective From
		...
		errormess &= this.verifyAddQuestionField("Effective From Date",
				"Sat Apr 30 2011");// Effective From Date entry is not a valid
									// date.(message 5 in the Spec)
		errormess &= this.verifyAddQuestionField("Effective To Date",
				"Mon Apr 4 2011");// Effective To Date entry is specified but is
									// not a valid date.(message 6 in the Spec)
		...
		if(!MiscFunctions.blockByDefect()){// DEFECT-31974 furture release
			...
			errormess &= this.verifyAddQuestionError(message4); // message4
		}
	...
	public void wrapParameters(Object[] param) {
		...
		consumable.name = "Auto_test_For_C_Verify";
		consumable.code = "005";
		consumable.description = "Auto_test_For_C_Verify";
	...
	private boolean verifyAddQuestionError(String error) {
		...
		if (error.equals(message)) {
			...
			logger.error("The error message '" + error
					+ "' on the add product question page displayed wrong. Actual is: " + message);
			return false;
		}
		return true;
	}
	...
	private void gotoConsumableQuestionPg() {
		...
		if(null == consumable.id || "".equals(consumable.id)) {
			...
			// add new consumable with name is "Auto_test_For_C_Verify"
