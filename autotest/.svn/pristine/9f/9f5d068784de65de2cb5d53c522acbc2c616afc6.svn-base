 ...
 *              Check point:cancel button work
...
public class VerifyCancel extends LicenseManagerTestCase{
	...
	private boolean verifyCancelQuestionSuccess(QuestionInfo question) {
		...
		String id = supplyQuestionPg.getProductQuestionAssignmentID(question);
		...
		logger.error("Can't cancel a new question for product which id is:"+id);
		return false;
	}
}
