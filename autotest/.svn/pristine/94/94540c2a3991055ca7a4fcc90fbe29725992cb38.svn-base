 ...
 * @Date  Apr 18, 2013
 */
public class VerifyDeclaStatement_CANAddr extends HFSKWebTestCase{
	...
	public void execute() {
		...
		resStatusPg.verifyDeclarStatErrorMsg(residStatus[2], IDENT_TYPE_OTHER, errMsg);
	...
	public void wrapParameters(Object[] param) {
		cus.email = "nonmohcan0001@test.com"; // with Canada not SK address
...
//		failPgSubTitle = "Your residency status could not be verified.*";
		failPgSubTitle = WebConfiguration.getInstance().getPropertiesValue(Conf.HFCustomer_prop, HFCustomerProperty.ResidenceCheck18);//("failureMsg.residencyDeclaration.resStatusDecl.residencyCheck.18.UWP_RD_0001_HFSK");
...
//		failPgInfo = "Important.*qualify as a " + residStatus[0] + " - " + IDENT_TYPE_RCMP + ".*home address has to be in Saskatchewan, Canada.*";
	...
	private void selectStatusWithoutIdentAndVerifyInfo() {
		...
		if (!result) {
			throw new ErrorOnPageException("There should be no identifier info input fields or declaration statement for account with Canada address");
		}
		logger.info("---Verify no identifier info input fields or declaration statement for account with Canada address");
	}
	...
	private void selectStatusWithIdentAndVerifyInfo() {
		...
		result &= resStatusPg.compareIdentAndDeclarInfoExist(residStatus[1], IDENT_TYPE_CANDL, true, false, true, false, null);
		...
		if (!result) {
			throw new ErrorOnPageException("Identifier info and declaration statement are not shown correctly for the account with Canada address! Check logger error!");
		}
		logger.info("--Verify Identifier info and declaration statement are shown correctly for account with Canada address");
	}
}
