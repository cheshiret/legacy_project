...
/**
 * @Description: It is verified edit waiting list of lottery
 * @LinkSetUp: none
 * @SPEC:[TC:044676] Edit Lottery Schedule - Waiting list 
 * @Task#:Auto-2057
 * @author Phoebe Chen
 * @Date  March 27, 2015
 */
public class VerifyEditWaitingList extends InventoryManagerTestCase {
	...
	public void execute() {
		...
		lotterySchedule.waitList = "ListForSearch02";  //Change to another waiting list
		im.editLotterySchedule(lotterySchedule, lottery.productCategory);
		//Verify waiting list updated successfully
		verifyLotterySheduleDetailInfo();
		
		
		lotterySchedule.isAddToWaitList = false; //Set waiting list to empty
		lotterySchedule.waitList = StringUtil.EMPTY; 
		im.editLotterySchedule(lotterySchedule, lottery.productCategory);
		//Verify waiting list updated successfully
		verifyLotterySheduleDetailInfo();
		
		
		lotterySchedule.isAddToWaitList = true; //Change waiting list back to original
	...
	private void verifyLotterySheduleDetailInfo(){
