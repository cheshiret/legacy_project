...
/**
 * @Description: 
 * 		1.create attribute fee schedule for slip product, Marina Rate Type of which is "Transient".
 * 
 * @Preconditions:
 * 		1.Slip product is created in NC contract(Mayo River State Park), PRD_CD="PZH". in table "d_inv_add_slip"(ID=160)
 * @SPEC:TC043402,Add Attr Fee Schedule - Transient
 * @Task#:AUTO-1328
 * 
 * @author pzhu
 * @Date Jan 7, 2013
...
public class AddTransient extends FinanceManagerTestCase {
	...
	private void verifyScheduleSlipPart(FeeScheduleData schedule) {
		...
		Assert.assertEquals("Compare rafting..",schedule.rafting,detailPg.getRafting());
		Assert.assertEquals("Compare daily fee..",Double.valueOf(schedule.slipFees.get(0).dailyNightlyFee),Double.valueOf(detailPg.getSlipDailyFee(schedule.slipPricingUnit).get(0)));
		logger.info("Passed!!!.....");
	}
