 ...
 * monthly required number of nights,it use customer rate and nightly
 * rate(length of stay:29)
 * 
 * @author Sophia
 * 
 */
public class WalkinWithRequiredMultiunitRates_5 extends
	...
	public void wrapParameters(Object[] param) {
		...
		feeData.departureDate = "Thu Apr 01 2010";
		Map<String, String> multi = new HashMap<String, String>();
		multi.put(RateType.KEY_CUSTOMERRATE, "2");
		feeData.units.add(multi);
		feeData.nights.add("5");
		feeData.time.add("3/27/2010");
	...
	public void verifyAttributeFee(FeeValidationData feeData) {
		...
		// calculate rule
		BigDecimal amount = feeCal.calculateBaseFee(feeData, 3, false).get(0);
		BigDecimal amount1 = feeCal.calculateBaseFee(feeData, 0, false).get(0);
		String fee = amount.toString() + feeData.schedules.get(0) + "Custom";
