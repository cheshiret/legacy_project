 ...
 * @Date  Dec 6, 2011
 */
public class VerifyRAFeeSchdSelectCreationForPOS extends FinanceManagerTestCase{
	...
	private void verifyProduct(String sub,String location, boolean isRevenue){
		...
		List<String[]> productDB=null;
		if(isRevenue){
			productDB=fnm.getProductNameUsingFeeSchdDetailPgFromDB(schema, "4", sub, "", location);
		}else{
			productDB=fnm.getProductNameUsingFeeSchdDetailPg(schema, "4", sub, location, "",true);
		}
...
//			if(!productGrp.contains(group[0].trim())){
//				throw new ErrorOnDataException("Product group don't contain the expect value:"+group[0]+".");
//			}
//		}
		//----------------------
		//comments by Jane: Below check points were invalid due to POS PCR
...
//		}
	...
	public void verifyProductAndTransType(){
		...
		this.verifyProduct("1", "1", false);// 1:POS sale
		...
		this.verifyProduct("2", "1", true);//2:Subscription Sale
		...
		this.verifyProduct("3", "1", true);//3: Donation
