...
/**
 * @Description:This case used to verify calculate replace privilege inventory transaction fee tax
 * @Preconditions:This customer must be NOT "Tax Exempt", this attribute could be configure at customer detail page
 * @LinkSetUp: D_HF_ADD_PRIVILEGE_PRD:id=3430
 *             D_HF_ASSI_PRI_TO_STORE:id=2630
 *             D_HF_ADD_PRVI_LICENSE_YEAR:id=3540
...
public class CalculateTax_ReplacePriInventory extends LicenseManagerTestCase{
	...
	private String saleLocation,tranTaxSchdID_tax1,tranTaxSchdID_tax2,replaceReason,replaceNote;
	...
	public void execute() {
		...
		tranTaxSchdID_tax1 = this.getTaxSchdID(privilege.privilegeId, FEETYPE_TRANFEE, REPLACE_PRIVILEGE_INVENTORY_ID,pricing_replacePriInv.transactionTaxes.get(0).getTaxName(),schema);
		//get vendor fee tax 2 schedule id
		tranTaxSchdID_tax2 = this.getTaxSchdID(privilege.privilegeId, FEETYPE_TRANFEE, REPLACE_PRIVILEGE_INVENTORY_ID,pricing_replacePriInv.transactionTaxes.get(1).getTaxName(),schema);
		...
		priFeesPage.verifyFeeInfo(pricing_replacePriInv.transactionTaxes.get(0).getTaxName(), tranTaxSchdID_tax1, tranTaxAmount_1, TRANNAME_REPLACE_PRIVILEGE_INVENTORY);
		priFeesPage.verifyFeeInfo(pricing_replacePriInv.transactionTaxes.get(1).getTaxName(), tranTaxSchdID_tax2, tranTaxAmount_2, TRANNAME_REPLACE_PRIVILEGE_INVENTORY);
		lm.gotoOrderCartPageFromFeeDetailPage();
		//verify tax total amount by tax name
		ordCartPg.verifyFeeAmountCorrect(pricing_replacePriInv.transactionTaxes.get(0).getTaxName(), true, String.valueOf(tranTaxAmount_1));
		ordCartPg.verifyFeeAmountCorrect(pricing_replacePriInv.transactionTaxes.get(1).getTaxName(), true, String.valueOf(tranTaxAmount_2));
	...
	public void wrapParameters(Object[] param) {
		...
		privilege.name = "TaxProcessPrivilege17";
		privilege.code = "PT7";
		...
		TaxInfo tax_tran_1 = pricing_replacePriInv.new TaxInfo();
		tax_tran_1.setIsFlat(true);
		tax_tran_1.setTaxName("TaxForPriviledge_Flat");
		tax_tran_1.setTaxCalculationRate("20");
		tranTaxAmount_1 = ((new BigDecimal(pricing_replacePriInv.transFee).
		...
		TaxInfo tax_tran_2 = pricing_replacePriInv.new TaxInfo();
		...
		tranTaxAmount_2 = ((new BigDecimal(pricing_replacePriInv.transFee).
				multiply(new BigDecimal(tax_tran_2.getRate()).movePointLeft(2))).
				setScale(2,BigDecimal.ROUND_HALF_UP)).doubleValue();
		
		pricing_replacePriInv.transactionTaxes.add(tax_tran_1);
		pricing_replacePriInv.transactionTaxes.add(tax_tran_2);
