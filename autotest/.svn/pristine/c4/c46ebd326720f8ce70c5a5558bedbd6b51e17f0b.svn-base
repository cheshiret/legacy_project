...
/**  
 * @Description: View privilege details
 * @Preconditions: 
 * @SPEC:  View privilege details
 * @Task#: Auto-871
 * @author jwang8  
 * @Date  Feb 21, 2012    
 */
public class ViewPriDetailsFromSearchPriPg extends LicenseManagerTestCase{
	
	@Override
	public void execute() {
		lm.loginLicenseManager(login);
		
		//Make privilege to cart.
		...
		//Get the privilege number.
		privilege.privilegeId = lm.getPrivilegeNumByOrdNum(schema, orderNum);
		//go to privilege search page.
		lm.gotoPrivilegeTabPage();
		//go to privilege detail page.
		lm.gotoPrivilegeDetailPageFromPriSearchPg(privilege.privilegeId);
