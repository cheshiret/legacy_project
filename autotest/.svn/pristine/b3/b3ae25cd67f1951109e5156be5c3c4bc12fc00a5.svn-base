 ...
 * @Date  Dec 6, 2011
 */
public class VerifyRAFeeSchdSelectCreationForVehicle extends FinanceManagerTestCase{
	...
	public void wrapParameters(Object[] param) {
		//initialize login finance manager loginInfo
	  	...
	  	schedule.productCategory="VehicleRTI";
	}
	
	private void verifyProduct(){
		List<String> productGrp=detailPg.getAllProductGroup();
		List<String> product=detailPg.getAllProduct();
		List<String[]> productGrpDB=fnm.getProductGroupNameUsingFeeSchdDetailPgFromDB(schema, "11", "");
		List<String[]> productDB=fnm.getProductNameUsingFeeSchdDetailPgFromDB(schema, "11", "", "", "1");
...
//		}
	...
	private void verifyVehicleType(){
		List<String> vehicleType=detailPg.getVehicleTypeValues();
		List<String> types=fnm.getVehicleTypeFromDB(schema);
		
		if(vehicleType==null||(vehicleType.size()-1)!=types.size()){
			throw new ErrorOnDataException("Vehicle Type value size is not correct.");
		}
		if(!vehicleType.contains("All") ||! "All".equalsIgnoreCase(vehicleType.get(0))){
			throw new ErrorOnDataException("Vehicle Type is not contain 'All' or the default value is not 'All'.");
		}
	}
	
	private void verifyTransType(){
