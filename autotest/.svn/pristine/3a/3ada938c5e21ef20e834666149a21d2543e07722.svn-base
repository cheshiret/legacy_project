...
/**
 * 
 * @Description: This case is used to verify whether editing an existing display sub-category successfully or not
 * @Preconditions:
 ...
 * @Date  Dec 29, 2011
 */
public class EditDisplaySubCategory extends LicenseManagerTestCase {
	...
	public void execute() {
		...
		//edit the new added display sub-category
		displaySubCategory.description = "Basic_Edit-"+DateFunctions.getCurrentTime();
		displaySubCategory.displayOrder = String.valueOf(Math.abs(new Long(DateFunctions.getCurrentTime()).intValue()));
		lm.editDisplaySubCategory(displaySubCategory);
		
		//verify the new edited display sub-category record list and detail info
	...
	public void wrapParameters(Object[] param) {
		...
		displaySubCategory.description = "Basic_Add_e-"+DateFunctions.getCurrentTime();
		displaySubCategory.displayOrder = String.valueOf(Math.abs(new Long(DateFunctions.getCurrentTime()).intValue()));
		...
		displaySubCategory.lastUpdatedUser = com.activenetwork.qa.awo.util.DataBaseFunctions.getLoginUserName(login.userName);
		displaySubCategory.lastUpdatedLocation = displaySubCategory.addLocation;
	}
