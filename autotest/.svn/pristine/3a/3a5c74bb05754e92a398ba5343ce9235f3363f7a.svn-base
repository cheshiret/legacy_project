...
/**
 * 
 * @Description: For award accepted by customer lottery application with group leader Applicant type, 
 * check lottery attributes and entered hunts info in account overview, order history and lottery application page
 * @Preconditions: 
 * @LinkSetUp:
 * d_web_hf_signupaccount:id=1370 --RCMP #|1R9Y4x4192
 * d_hf_add_privilege_prd:id=2640 --LAD,Lottery App Pri 04
 * d_hf_add_pricing:id=3772,3922--Priv, lottery
 * d_hf_assi_pri_to_store:id=1860
 * d_hf_add_prvi_license_year:id=2750
 * d_hf_add_qty_control:id=1840
 * d_hf_add_hunt:id=650 --LAH9
 * d_hf_add_hunt_license_year:id=760
 * d_hf_assign_priv_to_hunt:id=530
 * d_hf_add_hunt_quota:id=190
 * d_hf_add_hunt_location:id=70
 * d_hf_add_weapon:id=50
 * d_hf_add_lottery_prd:id=280 --WL8
 * d_hf_add_lottery_license_year:id=190
 * d_hf_add_lottery_quantity_control:id=190
 * d_hf_assi_hunts_to_lottery:id=250
 * d_hf_add_lottery_execution_config:id=170 --WebLottery08Config
 * d_hf_add_lottery_schedule:id=110 --WebLottery08Processing
 ...
 * @Date  Mar 9, 2014
 */
public class AwardAcceptedByCust_GroupLeader extends HFSKLotteryApplicationTestCase{

	@Override
	public void execute() {
		//Precondition: 
		// *Make Award Lottery application
		prepareLotteryAppInLM();
		//* Accept award in HFSK
		hf.invokeURL(url);
	...
	public void wrapParameters(Object[] param) {
		...
		cus.identifier.identifierNum = "1R9Y4x4192";
		...
		//lottery info
		lottery.setCode("WL8");
		lottery.setDescription("WebLottery08");
		...
		schedule.setExecutionConfig("WebLottery08Config");
		schedule.setLicenseYear(licenseYear.licYear); 
		schedule.setDescription("WebLottery08Processing");
		...
		privilege.licenseYear = "licenseYear.licYear";

		initializeAwardedLotteryHuntAttris();
	}

	public void generateLotteryAppAndItsOrderHistoryAttrs(){
		awardedHuntDetails = "Awarded Hunt Licence # "+licenseNum+awardedLotteryHuntAttris;
		lotteryDetails_lotteryApp = lottery.getDescription()+" \\("+lottery.getLicenseYear()+"\\) "+lotteryAppDate+" "+label_ApplicationNum+" "+lotteryOrderNum+" Status: "+AWARDEDACCEPTEDBYCUSTOMER_STATUS+" "+awardedHuntDetails+" Show Group Members.*";
		lotteryDetails_orderHistory = label_BigGameDrawOrderNum+" "+lotteryOrderNum+" "+lotteryAppDate+" "+lottery.getDescription()+" \\("+lottery.getLicenseYear()+"\\) Status: "+AWARDEDACCEPTEDBYCUSTOMER_STATUS+" "+awardedHuntDetails+" Show Group Members.*";
