 ...
 * @Date  Nov 14, 2012
 */
public class VerifyTransactionSelection_Site extends FinanceManagerTestCase {
	...
	private void verifyTransactionType() {
		...
		for (int i = 0; i < value.size(); i++) { // Change it due to the defect-40362 resolved as Won't fix
			if(i+1<value.size() && value.get(i).compareTo(value.get(i+1)) > 0){
				logger.info("Comparing Row#" + i + " value: " + value.get(i) + 
						" Row#" + i+1 + " value: " + value.get(i+1));
				throw new ErrorOnPageException(
						"Transaction Type element should order by alpha-numeric sorting in ascending order at row "+i);
			}
		}
	}
