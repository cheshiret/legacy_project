 ...
 * @Date  May 27, 2011
 */
public class LicMgrConsumableProductPricingPage extends LicMgrConsumableProductDetailsPage implements ILicMgrProductPricingPage{
	
	private static LicMgrConsumableProductPricingPage _instance = null;
	
	private LicMgrConsumableProductPricingPage() {}
	
	public static LicMgrConsumableProductPricingPage getInstance() {
		if(null == _instance) {
			_instance = new LicMgrConsumableProductPricingPage();
		}
	...
	public String getPricingID(PricingInfo pricing) {
		...
		logger.info("Get consumable pricing id=" + id);
	...
	public boolean comparePricingRecordInfo(PricingInfo pricing) {
		...
		col = table.findColumn(0, new RegularExpression("(Fiscal Year From)|(License Year From)", false));
		...
		col = table.findColumn(0, new RegularExpression("(Fiscal Year To)|(License Year To)", false));
		if(!pricing.licYearFrom.equals("All")) {
//			if(!table.getCellValue(row, col).trim().equals(pricing.licYearTo)){
			if(null == table.getCellValue(row, col) || !pricing.licYearTo.equals(table.getCellValue(row, col).trim())){
				result &= false;
