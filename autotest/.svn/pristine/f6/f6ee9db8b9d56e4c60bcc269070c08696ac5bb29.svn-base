...
/**
 * This is the hunts history list page in license manager, Admin(drop down list)-->Lotteries --- > Hunts --->click hunt id-->click 
 * 'Change History'
 * @author pchen
 * @date Sep 24, 2012
 */
public class LicMgrHuntHistoryPage extends LicMgrLotteriesCommonPage{
	private static LicMgrHuntHistoryPage _instance= null;
	private LicMgrHuntHistoryPage (){}
	
	public static LicMgrHuntHistoryPage getInstance(){
		if(null == _instance){
			_instance = new LicMgrHuntHistoryPage();
		}
	...
	public boolean exists() {
		return browser.checkHtmlObjectExists(".class", "Html.TABLE",".id","HuntInfo");
	}
	
	/**
	 * Click return to hunt details button
	 */
	public void clickReturnToHuntDetails(){
		browser.clickGuiObject(".class", "Html.A", ".text", "Return to Hunt Details",true);
	}
