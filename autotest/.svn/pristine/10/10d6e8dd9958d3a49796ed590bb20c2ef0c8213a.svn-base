...
/**
 * @Description: 1. Purchase a privilege(5 copys), reverse it, then purchase a boat, populate a voucher.
 ...
 * 					2. set 'TRUE' for UpperCase: <update X_PROP set value='true' where name like 'UppercaseOutput'>; handle in case.
...
public class VoucherEarnedOrder extends LicenseManagerTestCase {
	...
	public void execute() {
		...
		this.processOrderWithVoucher();
		this.vehicleOrder =	ordSumPg.getOrderNumOfProduct(vehicle.hullIdSerialNum);
		lm.finishOrder();
	
		//Go to receipt page, get confirmation letter.
		lm.gotoVehicleOrderDetailPage(this.vehicleOrder);
	...
	private void processOrderWithVoucher() {
		...
		OrmsProcessOrderCartPopupPage pinPopupPg = OrmsProcessOrderCartPopupPage.getInstance();

		orderCartPage.selectRadioVoucher();
		orderCartPage.clickProcessOrder();
		ajax.waitLoading();
		Object page = browser.waitExists(pinPopupPg, ordSumPg);
		if(page == pinPopupPg) {
			pinPopupPg.enterPIN(new Payment().pin);
			pinPopupPg.clickOK();
