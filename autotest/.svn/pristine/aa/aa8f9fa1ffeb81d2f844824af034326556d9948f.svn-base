...
public class VerifyReportDate extends ResourceManagerTestCase{
	...
	public void execute() {
		...
		// End Date minus Report Start Date is equal to 32 days
		//Per spec: End Date: Entered end date should be a date less than or equal to start date+31 days.
		//So we just check >=32 days from startDate.
		...
		// End Date minus Report Start Date is greater than 32 days
		rd.startDate = DateFunctions.getDateAfterToday(-33);
		rd.endDate = DateFunctions.getDateAfterToday(0);
		rm.verifyReportDate(rd.startDate, rd.endDate, errorMessage.get(1));
